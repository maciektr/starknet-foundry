...
impl Optimizer {
    ...
    pub fn optimize(
        ...
        if args.bruteforce {
            ...
            self.optimize_binsearch(args, ui)
        }
    }

    fn optimize_binsearch(
        &mut self,
        args: &OptimizeInliningArgs,
        ui: &Arc<UI>,
    ) -> Result<OptimalResult> {
        let mut low = self.min_threshold;
        let mut high = self.max_threshold;
        let mut best_valid: Option<OptimizationResult> = None;
        let mut iteration = 1;
        ...
        while low <= high {
            ...
            ui.print_blank_line();
            ui.println(&format!(
                "[{}] Binary search: testing threshold {} (range: {} - {})",
                iteration, mid, low, high
            ));

            let result = run_optimization_iteration(mid, args, &self.scarb_metadata, ui)?;
            ...
            let is_valid = result.tests_passed && result.error.is_none();
            self.results.push(result.clone());

            if is_valid {
                best_valid = Some(result);
                low = mid + 1;
            } else {
                if mid == 0 {
                    break;
    ...
    fn optimize_bruteforce(
        ...
        while current <= self.max_threshold {
            ...
            let result =
                run_optimization_iteration(current, args, &self.scarb_metadata, ui)?;
    ...
    pub fn print_results_table(&self, ui: &UI) {
        ui.println(&"┌──────────────┬─────────────────┬──────────────────┬──────────────┬────────┐".to_string());
        ui.println(&"│  Threshold   │    Total Gas    │  Contract Size   │    Felts     │ Status │".to_string());
        ui.println(&"├──────────────┼─────────────────┼──────────────────┼──────────────┼────────┤".to_string());
